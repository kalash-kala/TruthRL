...
def compute_score(solution_str, ground_truth, method="strict",
    ...
    # ------------------------------------------------------------------
    # G. Execute Judges in Parallel (Optimized)
    # ------------------------------------------------------------------
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        future_v = None
        if not is_abstention:
            future_v = executor.submit(get_vqa_judge_verdict, question_for_judge, acceptable_answers, predicted_answer)

        # 2. Reasoning Judge
        future_r = None
        answerability_upper = answerability.upper().strip() if answerability else ""
        if answerability_upper in ("ANSWERABLE", "UNANSWERABLE"):
            future_r = executor.submit(get_reasoning_compatibility, question_for_judge, reasoning_text, predicted_answer, answerability_upper)

        # 3. Visual Judge
        future_vis = executor.submit(get_visual_cue_support, question_for_judge, visual_cues, reasoning_text)

        # Collect results
        if future_v:
            verdict, explanation = future_v.result()
            if verdict == "CORRECT":
                is_correct = True
                accuracy = 1.0
        ...
        if future_r:
            reasoning_compatible, reasoning_quality, reasoning_explanation = future_r.result()
        
        visual_support_score, cue_scores, visual_explanation = future_vis.result()

    # Fallback if reasoning judge fails or wasn't run
    if answerability_upper in ("ANSWERABLE", "UNANSWERABLE") and reasoning_quality is None:
    ...
    # Visual support score and cue scores collected above in parallel block
